/**
 * TEOS-10: the Gibbs function of seawater.
 *
 * Everything this package reports is a derivative of one function. `gibbs`
 * evaluates the specific Gibbs energy g(SA, t, p) and its partial derivatives
 * to second order, from which density, entropy, enthalpy, sound speed, heat
 * capacity and the rest follow analytically -- see `properties.ts`.
 *
 * The two halves of it are separate standards. The pure-water part is
 * IAPWS-09 (a Gibbs function fitted to IAPWS-95); the saline part is
 * IAPWS-08, a polynomial in sqrt(SA/SSO), t and p. Their sum is the
 * TEOS-10 Gibbs function of IOC, SCOR and IAPSO (2010).
 *
 * **The coefficient blocks below are transcribed mechanically from GSW-C's
 * `gsw_oceanographic_toolbox.c` and must never be edited by hand.** There are
 * some two thousand literals in this file; a single mistyped digit changes an
 * answer by an amount no one would notice and no reviewer could see. What
 * guards them is `npm run test:teos10`, which checks every branch against the
 * GSW reference implementation -- so a coefficient that drifts fails a check
 * rather than quietly shifting the density of the ocean.
 *
 * Ice Ih has its own standard and its own file -- see `gibbs-ice.ts`.
 *
 * Renderer-independent by construction: no DOM, no fetch, no framework. The
 * atlas in `atlas.ts` is the one part that needs the network, and it is
 * injected rather than imported.
 */

import { SFAC } from './constants.ts';

/**
 * Specific Gibbs energy of seawater and its derivatives.
 *
 * `ns`, `nt` and `np` are the orders of the derivative with respect to
 * Absolute Salinity (g/kg), in-situ temperature (degrees C, ITS-90) and sea
 * pressure (dbar). Orders summing above two are not defined and return NaN.
 *
 * Units are J/kg divided by (g/kg)^ns (K)^nt (Pa)^np -- note the pressure
 * derivatives are per pascal, not per decibar, which is why the callers in
 * `properties.ts` never scale them.
 */
export function gibbs(ns: number, nt: number, np: number, sa: number, t: number, p: number): number {
  let x2 = 0, x = 0, y = 0, z = 0, g03 = 0, g08 = 0, return_value = 0.0;
  ...
  if (ns == 0  && nt == 0  && np == 0) {
      ...
      if (sa > 0.0)
          g08     = g08 + 851.226734946706*x2*Math.log(x);
      ...
      if (sa > 0.0)
          g08 = g08 + 1702.453469893412*Math.log(x);

      return_value        = 0.5*SFAC*0.025*g08;
      ...
      return_value = 0.25*SFAC*SFAC*g08;
...
/**
 * The second temperature derivative of the Gibbs function at the surface.
 *
 * Equal to `gibbs(0, 2, 0, SA, pt0, 0)`, and worth having separately only
 * because the potential-temperature iteration calls it twice per step.
 */
export function gibbsPt0Pt0(sa: number, pt0: number): number {
  let x2 = 0, x = 0, y = 0, g03 = 0, g08 = 0;
...
/**
 * The part of entropy that the potential-temperature iteration needs.
 *
 * Entropy is -g_T; these two drop the terms that cancel when the iteration
 * compares entropy at (t, p) against entropy at (pt0, 0), which is all
 * `pt0FromT` ever asks of them. Neither is entropy on its own -- for that,
 * see `entropy` in `properties.ts`.
 */
export function entropyPart(sa: number, t: number, p: number): number {
  let x2 = 0, x = 0, y = 0, z = 0, g03 = 0, g08 = 0;
...
export function entropyPartZeroP(sa: number, pt0: number): number {
  let x2 = 0, x = 0, y = 0, g03 = 0, g08 = 0;
